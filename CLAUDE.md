...
**Program builder** (`renderProgramForm` + `handleSaveProgram`): the module-level **`workoutData` scratch object is the source of truth** — every mutation (add/remove/reorder, and the inline name/sets editors) writes to it, the rendered list is only a projection of it (`rerenderWorkoutExercises`), and `handleSaveProgram` reads it. "Copy program" reuses the create form via `copySourceProgramId` (pre-fills from a source program, saves as a brand-new one). Editing is **blocked while a session for that program is in progress** (`canChangeProgram`), so a live session's template can never change underneath it. The same guard blocks **Restart** (`restartProgram`), which resets the active program to its first workout at cycle 1 without touching logged sessions or sets.

**Workout numbers are stable slot ids, not indexes.** Sessions reference them, so they are never renumbered and **can have gaps** (saving only emits workouts that have exercises). Never assume `1..N`: use `getWorkoutNumbers(program)` to traverse. `advanceWorkout` walks that list, and `handleSaveProgram` clamps `currentWorkout` forward if an edit removes the workout it pointed at. Editing a program must only write `{ name, workouts }` — `isActive`/`currentWorkout`/`completedCycles` belong to the *user's progress*, not the template.
